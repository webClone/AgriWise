...
def decode(raw: RawSensorMessage, strict: bool = False) -> DecoderResult:
    ...
    # Milesight AM/EM series payload mapping
    mapping = {
        "battery": ("battery_pct", "percent"),
        "temperature": ("air_temperature_c", "c"),
        "humidity": ("relative_humidity_pct", "percent"),
    }
    ...
    if result.ignored_keys and strict:
        raise ValueError(f"Milesight decoder: unknown payload keys: {result.ignored_keys}")
